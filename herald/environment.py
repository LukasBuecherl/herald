...
class HERALDEnvironment:
    ...
    def __init__(self, protein_name, protein_sequence, model, alphabet, clf):
        ...
        # hydrolysis conditions — fixed for now, will become part of action space
        self.ph = 7.0
    ...
    def step(self, action):
        ...
        The reward formula is:
            reward = 0.4 * avg_probability + 0.6 * max_probabilitys
        ...
        if len(peptides) == 0:
            # no valid peptides produced — penalize with zero reward
            reward = 0.0
        else:
            probabilities = [
    ...
    def render(self):
        ...
        print(
            f"Last action: {self.tried_combinations[-1]} | "
            f"Reward: {self.rewards_history[-1]:.3f}"
        )
